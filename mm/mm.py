...
def get_mm_type(f):
    header = read_mm_header(f)
    first_line = header[0].split()
    assert(first_line[0] == '%%MatrixMarket')
    assert(first_line[1] == 'matrix')
    assert(first_line[2] == 'coordinate')
    assert(first_line[4] == 'general')
    t = header[0].split()[3]
    assert(t in ['real', 'integer'])
    return float if t == 'real' else int
...
def get_total_counts_mm(matrix_file, features_file, barcodes_file):
    ...
    line_count = 0
    matrix_type = get_mm_type(matrix_file)
...
def mm_merge(matrix_list, feature_list, barcode_list):
    ...
    if len(matrix_list) == 1:
        ...
        for i in barcode_list:
            x = x.union(set(i))
        if not len(x) == sum([len(i) for i in barcode_list]):
            raise ValueError('Barcode lists are not unique')
